...
def run(rank, size, device_type, learning_rate, batch_size, epochs):
    """ Distributed Synchronous SGD with DDP """
    torch.manual_seed(1234)
    train_loader, _ = partition_dataset(batch_size)
    test_loader = get_test_loader(batch_size)
    device = torch.device(f"{device_type}:{rank}")
    #device = torch.device(f"{device_type}:{rank+1}")
    ...
    for epoch in range(epochs):
        ddp_model.train()  # Set the model to training mode
        epoch_loss = 0.0
        ...
        test_accuracy = evaluate(ddp_model, test_loader, device)
        print(
